  ...
  for (y = 0; y != height + 1; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      if (y >= 1 && x >= 1) {
        ...
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 2) = i_result_r.read();
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 1) = i_result_g.read();
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 0) = i_result_b.read();
      } 
    }
  }
  sc_stop();
